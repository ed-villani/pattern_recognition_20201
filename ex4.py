...
def main():
    ...
    for d in data_traning:
        if d[-1] == 1:
            c1_training.append(d[:13])
        else:
            c2_training.append(d[:13])
    ...
    for d in data:
        if d[-1] == 1:
            c1_random.append(d[:13])
        else:
            c2_random.append(d[:13])

    c1_random = array(c1_random)
    c2_random = array(c2_random)
    data_con = concatenate((c1_random, c2_random))
    pdf_c1 = [
        pdf_2(2, cov(c1_training.T), d, mean(c1_training.T, axis=1)) for d in data_con
    ]
    pdf_c2 = [
        pdf_2(2, cov(c2_training.T), d, mean(c2_training.T, axis=1)) for d in data_con
    ]
